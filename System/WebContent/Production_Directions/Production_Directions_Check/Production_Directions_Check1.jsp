...
<h1 align="center">생산지시서확인</h1>
	<form action="Production_Directions_Check2.jsp" >
	...
	<%
		...
		String PD_Date = null;
		String Production_Directions_Date = request.getParameter("Production_Directions_Date");
		String sql;
		
		try{
			conn = DriverManager.getConnection(url, "root", "postscript1!");
			stmt = conn.createStatement();
			sql = "select production_directions.Product_Product_Num, product.Product_Name, production_directions.PD_Date, production_directions.PD_Amount from production_directions join product on production_directions.Product_Product_Num = product.Product_Num where PD_Date like '" + Production_Directions_Date + "%' group by PD_Date";
			rs = stmt.executeQuery	(sql);
			
				while(rs.next()){
					PD_Date = rs.getString("PD_Date");
		%>
						<tr>
							<td>
								<input type="radio" name="PD_Date" value="<%=PD_Date%>">
							</td>
							<td>
								<input type="text" value="<%=PD_Date %>" disabled>
							</td>
						</tr>
		<%}%>
						<tr>
