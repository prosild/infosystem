...
<script>
	...
	<ul>
		...
		<li class="topMenuLi">
			...
			<ul class="submenu">
				<li>
					<a class="submenuLink" href="../Production_Directions_Register/Production_Directions_Register.jsp">생산지시서작성</a>
				</li>
				<li>
					<a class="submenuLink" href="Production_Directions_Check.jsp">생산지시서확인</a>			
				</li>
			</ul>
		</li>
	</ul>
...
<h1 align="center">생산지시서확인</h1>						
	<%
		...
		try{
			...
			sql = "select * from production_directions join product on Product_Num = Product_Product_Num	 where PD_Date='" + PD_Date + "'";
			rs = stmt.executeQuery	(sql);
			
				while(rs.next()){
					Product_Num = rs.getString("Product_Product_Num");
					Product_Name = rs.getString("Product_Name");
					PD_Amount = rs.getString("PD_Amount");
		 %>
		 			<table align="center" border="1">
						...
						<tr>
							...
							<td>
								<input type="text" value="<%=PD_Amount%>" disabled>
							</td>
						<%
						Connection conn2 = DriverManager.getConnection(url, "root", "postscript1!");
						Statement stmt2 = conn2.createStatement();
						String sql2 = "select * from product_stock where Product_Product_Num = '" + Product_Num + "'";
						ResultSet rs2 = stmt2.executeQuery(sql2);
					
						rs2.last();
							String PS_Amount = rs2.getString("PS_Amount");
				 %>
				 			<td>
				 ...
				 <%
					...
					<table align="center" border="1">	
						...
						<%
							...
							String sql1 = "select material_requirement.Material_Material_Num, material.Material_Name, material_requirement.MR_Amount from material_requirement join material on material_requirement.Material_Material_Num = material.Material_Num where Product_Product_Num='" + Product_Num + "'";
							ResultSet rs1 = stmt1.executeQuery(sql1);
						
							while(rs1.next()){
								Material_Num = rs1.getString("Material_Material_Num");
								Material_Name = rs1.getString("Material_Name");
								MR_Amount = rs1.getString("MR_Amount");
					 %>
					 		<tr>
					 			<td>
					 			...
					 			<td>
					 				<input type="text" value="<%=Integer.parseInt(MR_Amount) * Integer.parseInt(PD_Amount) %>" disabled>
					 			</td>
					 		<%
								Connection conn3 = DriverManager.getConnection(url, "root", "postscript1!");
								Statement stmt3 = conn3.createStatement();
								String sql3 = "select * from material_stock where Material_Material_Num = '" + Material_Num + "'";
								ResultSet rs3 = stmt3.executeQuery(sql3);
					
								rs3.last();
									String MS_Amount = rs3.getString("MS_Amount");
				 %>
				 			<td>
