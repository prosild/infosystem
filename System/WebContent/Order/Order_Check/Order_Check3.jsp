	...
	function back(){
		...
		<ul>
			...
			<li class="topMenuLi">
				...
				<ul class="submenu">
					<li>
						<a class="submenuLink" href="../Order_Register/Order_Register.jsp">주문등록</a>
					</li>
					<li>
						<a class="submenuLink" href="Order_Check.jsp">주문내역조회</a>
					</li>
				</ul>
			</li>
			<li class="topMenuLi">
...
<h1 align="center">주문내역조회</h1>
	...
	<%
		...
		String Order_Sum = null;
		String Order_Date = null;
		String Product_Num = null;
		String Product_Name = null;
		String Order_History_Amount = null;
		String Order_History_Sale_Price = null;
	...
	<%
		try{
			...
			sql = "select system.order.Order_Num, system.order.Customer_Cus_Num, system.order.Order_Date, system.order.Order_Sum, system.order.Order_Date, Customer.Cus_Name, order_history.Product_Product_Num, order_history.Order_History_Amount, order_history.Order_History_Sale_Price, Product.Product_Name from system.order join customer on system.order.customer_cus_num = customer.cus_num join order_history on system.order.Order_Num = order_history.Order_Order_Num join Product on order_history.Product_Product_Num = Product.Product_Num where Order_Num= '" + Order_Num[i] + "'";
			rs = stmt.executeQuery	(sql);
			
				rs.last();
					Order_Num[i] = rs.getString("Order_Num");
					Cus_Num = rs.getString("Customer_Cus_Num");
		 ...
		 %>
						...
						<tr>
							<td>
								<input type="text" value="<%=Order_Num[i] %>" disabled>
							</td>
							<td>
								<input type="text" value="<%= Cus_Num %>"  disabled>
							</td>
							<td>
								<input type="text" value="<%= Cus_Name %>" disabled>
							</td>
							<td>
								<input type="text" value="<%= Order_Sum %>" disabled>
							</td>
							<td>
								<input type="text" value="<%= Order_Date %>" disabled>
							</td>
						</tr>
		<%
		}
		...
		try{
			...
			rs = stmt.executeQuery	(sql);
				...
				while(rs.next()){
					Product_Num = rs.getString("Product_Product_Num");
					Product_Name = rs.getString("Product_Name");
					Order_History_Amount = rs.getString("Order_History_Amount");
					Order_History_Sale_Price = rs.getString("Order_History_Sale_Price");
		 %>
						<tr>
							<td>
								<input type="text" value="<%=Product_Num%>" disabled>
							</td>
							<td>
								<input type="text" value="<%=Product_Name%>" disabled>
							</td>
							<td>
								<input type="text" value="<%=Order_History_Amount%>" disabled>
							</td>
							<td>
								<input type="text" value="<%=Order_History_Sale_Price%>" disabled>
							</td>
							<td>
								<input type="text" value="<%=(Integer.parseInt(Order_History_Amount)) * (Integer.parseInt(Order_History_Sale_Price))%>" disabled>
							</td>
						</tr>
		<%}%>
			</table>
