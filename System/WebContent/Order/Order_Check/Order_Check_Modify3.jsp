...
<ul>
	...
	<li class="topMenuLi">
		...
		<ul class="submenu">
			<li>
				<a class="submenuLink" href="../Order_RegisterOrder_Register.jsp">주문등록</a>
			</li>
			<li>
				<a class="submenuLink" href="../Order_Check/Order_Check.jsp">주문내역조회</a>
			</li>
		</ul>
	</li>
	<li class="topMenuLi">
		<a class="menuLink" href="../../Product/Product_Register/Product_Register.jsp">제품관리</a>
		<ul class="submenu">
			<li>
...
<h1 align="center">주문등록</h1>
	<form action="Order_Check_Modify4.jsp">
		<table align="center" border="1">
			...
			<tr>
				<td>
					<input type="text" value="<%=Order_Num %>" disabled>
				</td>
				<td colspan="4" width="80%">
				</td>
			</tr>
	...
	<%
		...
		try {
			...
			sql = "SELECT * FROM Customer join system.order on system.order.Customer_Cus_Num = Customer.Cus_Num WHERE system.order.Order_Num='" + Order_Num + "'";
	...
	%>
			<tr>
				<td align="center"><input type="text" value="<%=Cus_Num%>" disabled><input type="hidden" name="Cus_Num" value="<%=Cus_Num%>"></td>
				<td align="center"><input type="text" value="<%=Cus_Name%>" disabled></td>
				<td align="center"><input type="text" value="<%=Cus_Tel%>" disabled></td>
				<td align="center"><input type="text" value="<%=Cus_Addr%>" disabled></td>
				<td align="center"><input type="text" value="<%=Cus_Email%>" disabled></td>
			</tr>
		<%}%>
		</table>
	<br>
	...
	<%	
		...
		try {
			conn = DriverManager.getConnection(url, "root", "postscript1!");
			stmt = conn.createStatement();
			sql = "SELECT * FROM Product WHERE Product_Num='" + Product_Num[i] + "'";
			rs = stmt.executeQuery(sql);
	...
	%>
			<tr>
				<td><input type="text" value="<%=Product_Num[i]%>" disabled><input type="hidden" name="Product_Num" value="<%=Product_Num[i]%>"></td>
				<td><input type="text" value="<%=Product_Name%>" disabled><input type="hidden" value="<%=Product_Name%>"></td>
				<td><input type="text" value="<%=Product_Amount[i]%>" disabled><input type="hidden" name="Product_Amount" value="<%=Product_Amount[i]%>"></td>
				<td><input type="text" value="<%=Product_Price[i]%>" disabled><input type="hidden" name="Product_Price" value="<%=Product_Price[i]%>"></td>
				<td><input type="text" value="<%=Integer.parseInt(Product_Amount[i]) * Integer.parseInt(Product_Price[i])%>" disabled></td>				
			</tr>	
		<%
				Order_Sum = Order_Sum + (Integer.parseInt(Product_Amount[i]) * Integer.parseInt(Product_Price[i]));
			}	
		} catch (SQLException e) {
			out.println(e);
		} finally {
	...
	<table width="54%" align="center">
		<tr>
			...
			<td align="right">
				<input type="text" value="<%=Order_Sum%>" disabled>
				<input type="hidden" name="Order_Sum" value="<%=Order_Sum %>">
			</td>
		</tr>
		<tr>
