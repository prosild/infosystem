...
<script>
	function modify(){
		location="Change_Product.jsp"	
	}
	function del(){
		location="Product_Delete.jsp"
	}
	...
	<ul>
		...
		<li class="topMenuLi">
			...
			<ul class="submenu">
				<li>
					<a class="submenuLink" href="../Product_Register/Product_Register.jsp">제품등록</a>
				</li>
				<li>
					<a class="submenuLink" href="../Product_Stock/Product_Stock_Check.jsp">제품재고확인</a>			
				</li>
				<li>
					<a class="submenuLink" href="../Product_SR/Product_SR_Register.jsp">제품입출고등록</a>			
				</li>
			</ul>
		</li>
		<li class="topMenuLi">
	...
	<%
		...
		try{
			...
			String sql = "SELECT * FROM Product WHERE Product_Num='" + Product_num + "'";
			rs = stmt.executeQuery(sql);
			
			while(rs.next()){
				Product_Num = rs.getString("Product_Num");
				Product_Name = rs.getString("Product_Name");
				if(Product_Num.isEmpty()){
					continue;
				}
				%>
				<form action="Product_Select_Modify.jsp" >
	...
	%>
		...
		url = "jdbc:mysql://localhost:3306/system?verifyServerCertificate=false&useSSL=true";
			...
			try {
				conn = DriverManager.getConnection(url, "root", "postscript1!");
				stmt = conn.createStatement();
				String sql = "SELECT Material_Requirement.Material_Material_Num, Material.Material_Name, Material_Requirement.MR_Amount from Material_Requirement inner join Material on Material_Requirement.Material_Material_Num = Material.Material_Num WHERE Product_Product_Num='" + Product_num + "'";
				rs = stmt.executeQuery(sql);

				while (rs.next()) {
					String Material_Num = rs.getString("Material_Material_Num");
					String Material_Name = rs.getString("Material_Name");
					String MR_Amount = rs.getString("MR_Amount");
	%>
		<table align="center" border="1">
			<tr>
				<td>
					<input type="checkbox" name="Material_Num" value="<%=Material_Num %>">
				</td>
				<td align="right">자재번호 :</td>
				<td><input type="text" value="<%=Material_Num%>" disabled>
					<input type="hidden" name="Material_Num" value="<%=Material_Num%>">
				</td>
				<td align="right">자재명 :</td>
				<td><input type="text" value="<%=Material_Name %>" disabled>
				</td>
				<td align="right">소요수량 :</td>
				<td><input type="text" value="<%=MR_Amount%>" disabled></td>
			</tr>
		</table>
		<%
				}
	...
	<form action="Product_Select_Add.jsp">
	<table align="right">
		<tr>
			<td>
				<input type="hidden" name="Product_Num" value="<%=Product_Num %>">
