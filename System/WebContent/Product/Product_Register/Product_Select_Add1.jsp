...
<script>
	function back(){
		location="Product_Select_Add.jsp"
	}
	...
	String[] Material_Name = new String[100];
	String sql;
	for(int i = 0; i < Material_Num.length; i++){
	try{
		conn = DriverManager.getConnection(url, "root", "postscript1!");
		stmt = conn.createStatement();
		sql = "SELECT * FROM Material where Material_Num='" + Material_Num[i] + "'";
		rs = stmt.executeQuery(sql);
		
		while(rs.next()){
			Material_Name[i] = rs.getString("Material_Name");
			}
		}
	...
	%>
				...
				<form action="Product_Select_Add2.jsp" >
					<table align="center" border="1">
						<tr>
							...
							<td>
								<input type="text" value="<%= Material_Num[i]%>"  disabled>
								<input type="hidden" name="Material_Num" value="<%=Material_Num[i] %>">
							</td>
							<td align="right">
								자재명 : 
							</td>
							<td>
								<input type="text" value="<%= Material_Name[i] %>" disabled>
							</td>
							<td>
								소요수량 : 
							</td>
							<td>
								<input type="text" name="MR_Amount">
							</td>
						</tr>
					</table>
		<% } %>
